...
interface Track {
  ...
  cover: string;
  preview: string;
  isCustom?: boolean; // НОВОЕ ПОЛЕ: Флаг для загруженных MP3
}
...
export default function App() {
  ...
  const parseDeezerTracks = (data: any[]) => data.filter((t: any) => t.preview).map((t: any) => ({
    id: t.id, title: t.title, artist: t.artist.name, cover: t.album.cover_xl || t.album.cover_medium, preview: t.preview, isCustom: false
  }));
  ...
  const cachePlaylistForOffline = async (playlist: Playlist) => {
    try {
      ...
      for (const track of playlist.tracks) {
        ...
        if (!match) {
          try { 
            await cache.add(track.preview); 
          } catch (e) { 
            console.warn('Не удалось закешировать трек (возможно CORS)', track.preview); 
          }
        }
        count++;
  ...
  const generateCards = async () => {
    ...
    for (let i = 0; i < cardsCount; i++) {
      // Алгоритм берет СЛУЧАЙНЫЕ 24 трека из всего плейлиста (даже если там 80 треков)
                        ...
                        return (
                          <div key={i} style={{ backgroundColor: template.config.gridColor }} className="rounded-lg border border-white/10 flex flex-col items-center justify-center text-center p-1.5 print:p-1 overflow-hidden backdrop-blur-sm">
                            {isFree ? <div style={{ color: template.config.accentColor }} className={`font-black uppercase leading-tight ${layoutNum === 4 ? 'text-xs' : 'text-xl'}`}>{template.config.centerText}</div> : (
                              <><div className={`font-bold leading-tight opacity-90 line-clamp-3 break-words w-full px-0.5 ${layoutNum === 4 ? 'text-[8px]' : 'text-[13px]'}`}>{cell.title}</div>{template.config.showArtist && <div style={{ color: template.config.accentColor }} className={`font-medium leading-tight opacity-90 italic mt-0.5 line-clamp-2 break-words w-full px-0.5 ${layoutNum === 4 ? 'text-[6px]' : 'text-[11px]'}`}>{cell.artist}</div>}</>
                            )}
                          </div>
                        )
                      })}
  ...
  if (hostSession) {
      ...
      return (
        <div className="fixed inset-0 bg-black text-white z-[100] flex flex-col items-center justify-center animate-in fade-in duration-500 group overflow-hidden">
          ...
          <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-50 bg-black/60 p-3 rounded-[2.5rem] backdrop-blur-md border border-white/10 shadow-2xl">
            <button onClick={() => setHideTrackInfo(!hideTrackInfo)} className={`flex items-center gap-3 px-8 py-4 rounded-full font-bold text-xl transition-all shadow-xl ${hideTrackInfo ? 'bg-purple-600 hover:bg-purple-500 text-white' : 'bg-gray-800 hover:bg-gray-700 text-gray-300'}`}>
    ...
    return (
      <div className="fixed inset-0 bg-gray-950 text-white z-50 flex flex-col font-sans animate-in zoom-in-95 duration-300">
        ...
        {autoWinners.length > 0 && (
          <div className="absolute top-24 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-green-500 text-white px-8 py-3 rounded-full font-black text-2xl shadow-[0_0_50px_rgba(34,197,94,0.5)] z-[150] animate-bounce cursor-pointer hover:scale-105 transition" onClick={() => setAutoWinners([])} title="Кликнуть чтобы скрыть">
      ...
      return (
        <div className="animate-in slide-in-from-right-8 duration-300 flex flex-col h-full">
          ...
          <div className="flex items-end justify-between mb-8 border-b border-gray-800 pb-8">
            ...
            <button onClick={(e) => deleteGame(currentGame.id, e)} className="flex items-center gap-2 px-4 py-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-xl transition font-bold">
              <Trash2 size={20} /> Удалить мероприятие
            </button>
          </div>
                  ...
                  return (
                    <div key={round.id} className="bg-gray-900 border border-gray-800 rounded-2xl p-6 flex flex-col group relative shadow-lg">
                      ...
                      <div className="mt-auto flex gap-3">
                        ...
                        <button onClick={() => { if(templates.length > 0 && !selectedTemplateId) setSelectedTemplateId(templates[0].id); setCardGeneratorSetup({game: currentGame, round}); }} disabled={!playlist || playlist.tracks.length < 24} className="flex-1 bg-gray-800 hover:bg-gray-700 text-white py-3 rounded-xl font-bold flex items-center justify-center gap-2 transition disabled:opacity-50">
            ...
            {currentPlaylist.tracks.length === 0 ? (
              ...
              <div className="flex flex-col gap-2">
                <div className="text-sm text-gray-500 mb-2 font-medium bg-gray-900 p-3 rounded-lg border border-gray-800">
                  <Timer size={16} className="inline mr-2 -mt-1"/> Подсказка: При генерации бланков алгоритм возьмёт случайные 24 трека из всех загруженных сюда. Вы можете загрузить 60-80 треков для разнообразия.
                </div>
              ...
              <div className="pt-4 border-t border-gray-800">
                ...
                {editingTemplate.config?.backgroundImageUrl ? (
                  <div className="relative rounded-xl overflow-hidden border border-gray-700 mb-4">
                    ...
                    <button onClick={() => setEditingTemplate({...editingTemplate, config: {...editingTemplate.config!, backgroundImageUrl: undefined}})} className="absolute top-2 right-2 p-1.5 bg-red-600 rounded-lg hover:bg-red-500 transition shadow-lg"><X size={16} /></button>
                  </div>
                ) : null}
              ...
              <div className="space-y-3 pt-4 border-t border-gray-800">
                ...
                {editingTemplate.config?.showQR && (
                  <div className="mt-3 animate-in fade-in slide-in-from-top-2 duration-200">
                    ...
                    <input type="url" value={editingTemplate.config?.qrUrl || ''} onChange={e => setEditingTemplate({...editingTemplate, config: {...editingTemplate.config!, qrUrl: e.target.value}})} className="w-full bg-gray-800 border border-gray-700 rounded-xl px-4 py-3 text-white focus:border-purple-500 outline-none placeholder:text-gray-600" placeholder="https://t.me/мой_канал или сайт" />
                  </div>
                )}
              </div>
