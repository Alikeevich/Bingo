 ...
 * «Вся карточка» — до самого конца очереди: там побеждает последняя клетка.
 * У линий сверху есть предел. Чтобы карточка выиграла именно на этой песне, все
 * ОСТАЛЬНЫЕ линии должны быть ещё не закрыты, а для этого в каждой из них нужна
 * клетка с песней «после». Одна клетка перекрывает сразу ряд, столбец и иногда
 * диагональ, так что пяти песен в хвосте достаточно — проверено перебором всех
 * возможных целей для очередей от 30 до 120 песен.
 */
const LATE_BUDGET = 5;
